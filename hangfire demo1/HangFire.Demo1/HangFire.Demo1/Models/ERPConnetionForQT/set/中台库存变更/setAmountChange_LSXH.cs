...
{
    /// <summary>
    /// 库存变更 零售销货 taobao.retail.ifashion.itemamount.change
    /// isonline 线下的零售单据
    /// 没有同步过的单据 is_move -> 0  已经同步过的单据 is_move  -> 1 已经上传库存变更 is_move -> 2
    /// </summary>
    public class setAmountChange_LSXH : StatusTask<QT_SPKCB>, IBillType
    {
		public override void Run()
		{
			...
			{
				arg = string.Format(" AND CONVERT(BIGINT,LastChanged) > {0} ", base.Config.pullbilldatetime);
			}  
			...
			if (dataTable != null && dataTable.Rows.Count > 0)
			{
				...
				for (int i = 0; i < count; i += 100)
				{
					...
					{
						this.sendLSXH_OffPay(drs, LastChanged);
					});
